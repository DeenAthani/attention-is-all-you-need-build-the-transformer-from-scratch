...
def build_token_to_id_vocab(sentences, specials=('<pad>', '<bos>', '<eos>', '<unk>')):
    """
    Maps unique tokens to integer IDs, starting with specials then corpus words.
    
    Assigns sequential IDs (0 to len(specials)-1) to the given special tokens,
    then adds unique tokens from `sentences` in first-seen order starting at 
    ID len(specials). 

    Args:
        sentences (list of str): Whitespace-tokenized corpus sentences.
        specials (tuple of str, optional): Special tokens to inject first.
...
def compute_positional_div_term(d_model):
    ...
    return torch.exp(even_indices * (-math.log(10000.0)/d_model))
...
def softmax_attention_weights(masked_scores):
    ...
    all_masked = ~torch.isfinite(masked_scores).any(dim=-1, keepdim=True)
